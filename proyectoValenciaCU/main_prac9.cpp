...
void InitGL ( )     // Inicializamos parametros
{
	...
	t_hoja.LoadTGA("hoja.tga");
	t_hoja.BuildGLTexture();
	t_hoja.ReleaseImage();

	t_manza.LoadTGA("manza.tga");
	t_manza.BuildGLTexture();
	t_manza.ReleaseImage();
...
void display ( void )   // Creamos la funcion donde se dibuja
{
	...
	glRotatef(g_lookupdown,1.0f,0,0);
		...
		glPushMatrix();		
			...
			//	glColor3f(1.0,0.0,0.0);//Todos los elementos para abajo se pintaran de este color
			//	miFigura.cono(4.0,2.0,8,0);
			//glPopMatrix();

			//miFigura.esfera(4.0,16.0,16.0,0);
			...
			glPushMatrix(); //Esfera o toroide
				...
				glTexGeni(GL_S, GL_TEXTURE_GEN_MODE,GL_SPHERE_MAP); //generar texturizado automatico, el ultimo parametro es el mas importante ;) 
			...
			glTranslatef(-5,2,-5);
				...
				glTexGeni(GL_S, GL_TEXTURE_GEN_MODE,GL_EYE_LINEAR); 
				glTexGeni(GL_T, GL_TEXTURE_GEN_MODE,GL_EYE_LINEAR);
				miFigura.cono(2.5,2.0,100,t_hoja.GLindex);//Parte arriba
				glDisable(GL_TEXTURE_GEN_S);
				glDisable(GL_TEXTURE_GEN_T);
			glTranslatef(0,-2,0);
				glEnable(GL_TEXTURE_GEN_S);
				...
				glTexGeni(GL_S, GL_TEXTURE_GEN_MODE,GL_EYE_LINEAR); 
				glTexGeni(GL_T, GL_TEXTURE_GEN_MODE,GL_EYE_LINEAR); 
				miFigura.cilindro(.5,2,50,t_arbol.GLindex);//Tronco
				glDisable(GL_TEXTURE_GEN_S);
				glDisable(GL_TEXTURE_GEN_T);
			...
			glColor3f(1,0,0);
				...
				glTexGeni(GL_S, GL_TEXTURE_GEN_MODE,GL_EYE_LINEAR); 
				glTexGeni(GL_T, GL_TEXTURE_GEN_MODE,GL_EYE_LINEAR);
				miFigura.esfera(0.3,20,20,t_manza.GLindex);//esfera uno
			glTranslatef(3.5,0,0);
			miFigura.esfera(0.3,20,20,t_manza.GLindex);//esfera udos
			glTranslatef(-1.75,0,1.75);
			miFigura.esfera(0.3,20,20,t_manza.GLindex);//esfera tres
			glTranslatef(0,0,-3.5);
			miFigura.esfera(0.3,20,20,t_manza.GLindex);//esfera cuarto
			glTranslatef(0,1,0.8);
			miFigura.esfera(0.3,20,20,t_manza.GLindex);//esfera cinco/*
			glTranslatef(-0.5,0,1.75);
			miFigura.esfera(0.3,20,20,t_manza.GLindex);//esfera seis
			glTranslatef(0,.5,-.5);
			miFigura.esfera(0.3,20,20,t_manza.GLindex);//esfera siete
